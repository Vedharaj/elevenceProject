...
export default function SprintsPage() {
  ...
  // Delete Sprint
  const openDeleteConfirm = (sprint: any) => {
    ...
    <div className="min-h-screen bg-[#F7F8F9] p-4 sm:p-6 lg:p-8 scrollbar-container">
            ...
            <section className="space-y-4">
              ...
              {activeSprint ? (
                <SprintCard
                  ...
                  onDelete={() => openDeleteConfirm(activeSprint)}
                />
              ) : (
                  ...
                  {plannedSprints.map((sprint) => (
                    <div key={sprint.id} className="w-[320px] sm:w-[420px] shrink-0">
                      <SprintCard
                        ...
                        onDelete={() => openDeleteConfirm(sprint)}
                      />
                    </div>
                  ))}
                </div>
      ...
      <Dialog open={createModalOpen} onOpenChange={setCreateModalOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button className="bg-[#0C66E4] text-white hover:bg-[#0055CC]" onClick={handleCreateSprint}>
              Create Sprint
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={startModalOpen} onOpenChange={setStartModalOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setStartModalOpen(false)}>Cancel</Button>
            <Button className="bg-[#0C66E4] text-white hover:bg-[#0055CC]" onClick={handleStartSprint}>
              Start Sprint
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <p className="text-sm text-[#42526E] py-2">
            Are you sure you want to delete <strong>{sprintToDelete?.name}</strong>? All issues inside this sprint will be returned to the Backlog.
          </p>
          <DialogFooter>
        ...
        {sprint.status !== "COMPLETED" && (onStart || onComplete || onEdit || onDelete) && (
          <div className="flex items-center gap-1.5">
            {sprint.status === "PLANNED" && onStart && (
              <Button size="sm" className="bg-[#0C66E4] hover:bg-[#0055CC] text-white font-semibold flex items-center gap-1" onClick={onStart}>
                <Play className="h-3 w-3" /> Start
              </Button>
            )}
            {sprint.status === "ACTIVE" && onComplete && (
              <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold flex items-center gap-1" onClick={onComplete}>
                <CheckCircle className="h-3.5 w-3.5" /> Complete
              </Button>
            )}
            {sprint.status === "PLANNED" && onEdit && (
              <Button size="icon" variant="ghost" className="h-8 w-8 text-[#626F86]" onClick={onEdit}>
